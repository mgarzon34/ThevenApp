...
public class EtiquetaNodoService {
    ...
    public static Label crearEtiquetaNodo(PuntoConexion punto, Pane zonaDibujo) {
        ...
        punto.nodoProperty().addListener((obs, oldVal, newVal) -> updateText.run());
        //Cambios de Net y del id de Net
        punto.netProperty().addListener((obs, oldVal, newVal) -> {
        	var props=punto.getProperties();
        	@SuppressWarnings("unchecked")
			var oldListener=(ChangeListener<Number>) props.get("netIdListener");
        	if(oldVal!=null && oldListener!=null) {
        		oldVal.idProperty().removeListener(oldListener);
        		props.remove("netIdListener");
        	}
        	if(newVal!=null) {
        		ChangeListener<Number> l=(ob, oldv, newv) -> updateText.run();
        		newVal.idProperty().addListener(l);
        		props.put("netIdListener", l);
        	}
        	updateText.run();
        });
        if(punto.getNet()!=null) {
        	ChangeListener<Number> l=(obs, oldVal, newVal) -> updateText.run();
        ...
        punto.sceneProperty().addListener((obs, oldVal, newVal) -> { if(newVal!=null) updatePos.run();});
        punto.boundsInParentProperty().addListener((obs, oldVal, newVal) -> updatePos.run());
