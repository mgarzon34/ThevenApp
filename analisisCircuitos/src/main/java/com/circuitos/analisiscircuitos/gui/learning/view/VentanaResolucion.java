...
/**
 * Clase que gestiona la ventana flotante de resolución de ejercicios.
 * 
...
public class VentanaResolucion {
	...
	public VentanaResolucion(Ejercicio ejercicio, Runnable onVolver, Runnable onExito) {
		...
		this.btnVerificar=new Button("✅ Verificar Respuestas");
		this.btnVolver=new Button("🔙 Volver a E-Learning");
	...
	/**
	 * Crea la ventana de resolución de ejercicios pidiendo los valores a calcular según
	 * el tipo de análisis pedido en el enunciado (Thevenin, Norton o ambos).
	 */
	private void inicializarVentana() {
		stage.setTitle("Resolviendo: "+ejercicio.getTitulo());
		stage.initModality(Modality.NONE); 		//Permite tocar la ventana de atrás
	 ...
	 * @param row					Integer con el número de la fila
	 * @param key					Clave o tipo de valor (Vth, Rth, In, Rn)
	 * @param labelText				Etiqueta (Label) del cuadro de texto
	 * @param unidades				String con las unidades según el tipo de valor
	 */
	...
	/**
	 * Valida las respuestas dadas por el usuario con los valores reales de resolución del circuito.
	 */
	private void validarRespuestas() {
		try {
			...
			if(ok) {
				lblResultado.setText("¡CORRECTO! 🎉 Ejercicio resuelto.");
				...
				lblResultado.setText("Incorrecto. Revisa unidades y cálculos.");
				lblResultado.getStyleClass().add("ventana-resolucion-incorrecto");
				stage.sizeToScene();
			}
		} catch(NumberFormatException e) {
			lblResultado.setText("Error: Introduce números válidos.");
	...
	/**
	 * Comprueba el valor dado por el alumno con la solución real después de convertir
	 * el valor dado por el alumno a factor 1 (V, A, Ω) con un error <0.1 o <5%.
	 * 
	 * @param key						Tipo de valor (Vth, Rth, In, Rn)
	 * @param solucionReal				Solución real
	...
	/**
	 * Obtiene el factor de conversión según la unidad (kilo, Mega o mili).
	 * 
	...
	/**
	 * Muestra la ventana de resolución de ejercicios.
	 */
	public void mostrar() {
		stage.show();
	}
