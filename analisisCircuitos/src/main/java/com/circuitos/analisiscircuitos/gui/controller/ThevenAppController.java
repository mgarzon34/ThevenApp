...
public class ThevenAppController {
	...
	private void configurarOpcionesVisuales() {
		...
		tabPanePrincipal.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
			...
			if(newVal==tabAnalisis) {
				panelOpcionesVisualesController.configurarModoAnalisis();
			} else if(newVal==tabAprendizaje) {
	...
	private void configurarTabs() {
		tabPanePrincipal.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
			...
			boolean esAnalisis=newVal==tabAnalisis;
			if(esAnalisis) {
			...
			boolean esDiseno=newVal==tabDiseno;
			if(esDiseno) {
	...
	/**
	 * Acción del menú para abrir el documento PDF con el manual de usuario.
	 * 
	 * @param event		Evento de apertura
	 */
	@FXML
	private void onAbrirManual(ActionEvent event) {
		String ruta="/com/circuitos/analisiscircuitos/manuales/manual_usuario.pdf";
		String userHome=System.getProperty("user.home");
		File folder=new File(userHome, "ThevenApp");
		File temp=new File(folder, "Manual_Usuario_tmp.pdf");
		try {
			if(Desktop.isDesktopSupported()) {
				if(!folder.exists()) {
					folder.mkdirs();
				}
				try (InputStream pdfSt=getClass().getResourceAsStream(ruta)) {
					if(pdfSt==null) {
						UIHelper.mostrarError("No se encontró el archivo del manual.");
						return;
					}
					Files.copy(pdfSt, temp.toPath(), StandardCopyOption.REPLACE_EXISTING);
				}
				Desktop.getDesktop().open(temp);
				temp.deleteOnExit();
				logger.info("Manual de usuario abierto con éxito");
			} else {
				UIHelper.mostrarError("El sistema no soporta la apertura de archivos externos.");
			}
		} catch(IOException e) {
			e.printStackTrace();
			logger.log(Level.SEVERE, "Error al intentar abrir el manual: ", e);
			UIHelper.mostrarError("Error al intentar abrir el manual: "+e.getMessage());
		}
	}
		...
		Platform.runLater(() -> {
			...
			if(root.getScene()!=null) {
				...
				root.sceneProperty().addListener((obs, oldVal, newVal) -> {
					if(newVal!=null) registrarAtajos(newVal);
				});
			}
		});
	}
